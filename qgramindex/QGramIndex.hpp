...
public:
    ...
    */
    ...
    /**
      ...
      @note The order of indices returned by this function is undefined, i.e. you may not rely on any
            specific order.
...
private:
   // YOUR PRIVATE MEMBER FUNCTIONS and VARIABLES HERE
    ...
    /**
    * @brief Computes a bit mask for q-gram hashing.
    *
    * The mask keeps only the lowest 2*q bits of a hash value,
    * ensuring that rolling hashes remain within the valid range
    * [0, 4^q).
    *
    * @param _q Length of the q-gram
    * @return Bit mask with the lowest 2*q bits set
    */
    uint32_t bit_mask(uint8_t _q);

    /**
    * @brief Counts all q-grams in the text using a rolling hash.
    *
    * Iterates over the text and increments the count for each
    * occurring q-gram hash. The result is stored in the count array.
    *
    * @param h Initial hash value of the first q-gram
    * @param text Input text (genome)
    */
    void count_qgram(uint32_t h, std::string const&text);
    /**
     * @brief Builds the cumulative sum array from q-gram counts.
     *
     * Transforms the count array into a prefix-sum (directory) array
     * that stores the start indices of each q-gram bucket in the suffix array.
     */
    void cumulative_sum();
    /**
     * @brief Fills the suffix array using the cumulative directory.
     *
     * Inserts the start positions of all q-grams into the suffix array
     * according to their hash values, using the directory as bucket boundaries.
     *
     * @param h Initial hash value of the first q-gram
     * @param text Input text (genome)
     */
    void fill_dir_sa(uint32_t h, std::string const&text);
